...
regress_bias <- function(context_sum){
  ...
  # mutate(A = log(plus_counts+1)+log(minus_counts+1), M = log(plus_counts+1)-log(minus_counts+1)) %>%
  #   lm(M ~ active + eid  + txType + A + bias_p + bias_d + bias_p2 + bias_d2,data=.)%>%
  # summary()
  # 
  # out_new_all = context_sum%>%
  # mutate(A = log(plus_counts+1)+log(minus_counts+1), M = log(plus_counts+1)-log(minus_counts+1)) %>%
  #   lm(M ~ active * eid  + A + bias_p + bias_d ,data=.)%>%
  # summary()
  
  
  #   out_new_all = context_sum%>%
  # mutate(A = log(plus_counts+1)+log(minus_counts+1), M = log(plus_counts+1)-log(minus_counts+1)) %>%
  #   lm(M ~ active * eid  + A + bias_p2 + bias_d2 ,data=.)%>%
  # summary()
  
  out_new_all = context_sum%>%
  mutate(A = log(plus_counts+1)+log(minus_counts+1), M = log(plus_counts+1)-log(minus_counts+1)) %>%
  lm(M ~ active + eid + txType + bias_p2 + bias_d2,data=.)%>%
  summary()
  # 
  #   out_new_all = context_sum%>%
  # mutate(A = log(plus_counts+1)+log(minus_counts+1), M = log(plus_counts+1)-log(minus_counts+1)) %>%
