...
summarize_bursting_events <- function(info_object, bursting_events = F, whole_region = T){
  ...
  top_bursting_object = test_sc_object%>% 
    group_by(eid) %>% 
    summarise(total_counts_max = max(total_counts),
              total_counts_sum = sum(total_counts),
              max_TPM = max(TPM),
              mean_TPM = mean(TPM),
              plus_counts_mean = mean(plus_counts), 
              minus_counts_mean =mean(minus_counts), 
              ncell = n())
  ...
  top_bursting_object = top_bursting_object %>% 
     mutate(txType = eid %>% extract_feature_from_info_object(eids = .,info_object =info_object ,feature_name = 'txType'),
            bulk_score = eid %>% extract_feature_from_info_object(eids = .,info_object =info_object ,feature_name = 'score'))
...
a = summary_bursting_t24 %>%
  mutate(bulk_score = as.numeric(bulk_score),
         Mean_burst_size = total_counts_sum/ncell) %>%
  ggplot(aes(x = log(ncell), y = log(mean_TPM)))+
  geom_point(alpha = 0.1)+
  geom_smooth(method = 'lm')+
  stat_poly_eq(formula = y ~ x, 
                aes(label = paste(..eq.label.., ..rr.label..,..p.value.label..,sep = "~~~")), 
...
ggsave('Jun12_histogram_direction_vs_freq_fractions.png', plot = last_plot(), path = 'C:/Users/evans/Documents/enhancer_project/report/figures\ and\ tables/', scale = 1,dpi = 300, width = 2, height=4, limitsize = FALSE)
```
